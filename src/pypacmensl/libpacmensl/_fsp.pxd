...
cdef extern from "pacmensl.h" namespace "pacmensl":
    cdef cppclass SensModel(Model):
        ...
        void *dprop_x_args_
        void *dprop_t_args_
        vector[vector[int]] dprop_t_sp_
        vector[vector[int]] dprop_x_sp_

        SensModel()

        SensModel(int num_parameters,
                  arma.Mat[int] stoichiometry_matrix,
                  vector[int] tv_reactions,
                  PropTFun & prop_t,
                  PropXFun & prop_x,
                  vector[PropTFun] & dprop_t,
                  vector[vector[int]] & dprop_t_sp,
                  vector[PropXFun] & dprop_x,
                  vector[vector[int]] & dprop_x_sp)
